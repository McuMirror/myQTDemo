...
void Widget::newParticipant(QString userName,QString localHostName,QString ipAddress)
{
    ...
    if(bb)
    {
        ...
        ui->textBrowser->append(QString(u8"%1 在线！").arg(userName));
        ui->onlineUser->setText(QString(u8"在线人数：%1").arg(ui->tableWidget->rowCount()));
        sendMessage(NewParticipant);
    }
}
...
void Widget::participantLeft(QString userName,QString localHostName,QString time)
{
    ...
    ui->textBrowser->setTextColor(Qt::gray);
    ui->textBrowser->setCurrentFont(QFont("Times New Roman",10));
    ui->textBrowser->append(QString(u8"%1 于 %2 离开！").arg(userName).arg(time));
    ui->onlineUser->setText(QString(u8"在线人数：%1").arg(ui->tableWidget->rowCount()));
}
                            ...
                            QString clientAddress,QString fileName)
{
    ...
    {
        int btn = QMessageBox::information(this,u8"接受文件",
                                           QString(u8"来自%1(%2)的文件：%3,是否接收？")
                                           .arg(userName).arg(serverAddress).arg(fileName),
                                           QMessageBox::Yes,QMessageBox::No);
        if(btn == QMessageBox::Yes)
        {
            QString name = QFileDialog::getSaveFileName(0,u8"保存文件",fileName);
...
void Widget::on_sendfile_clicked()
{
    ...
    {
        QMessageBox::warning(0,u8"选择用户",u8"请先从用户列表选择要传送的用户！",QMessageBox::Ok);
        return;
    }
...
bool Widget::saveFile(const QString &fileName)//保存文件
{
    ...
    {
        QMessageBox::warning(this,u8"保存文件",
                             QString(u8"无法保存文件 %1:\n %2").arg(fileName)
