...
void TcpServer::sendMessage()
{
    ...
    ui->serverStatusLabel->setText(QString(u8"开始传送文件 %1 ！").arg(theFileName));
...
void TcpServer::updateClientProgress(qint64 numBytes)//更新进度条
{
   ...
   ui->serverStatusLabel->setText(QString(u8"已发送 %1MB (%2MB/s) \n共%3MB 已用时:%4秒\n估计剩余时间：%5秒")
   ...
   //num.sprintf("%.1f KB/s", (bytesWritten*1000) / (1024.0*time.elapsed()));
    ...
    {
        ui->serverStatusLabel->setText(QString(u8"传送文件 %1 成功").arg(theFileName));
    }
}
...
void TcpServer::on_serverOpenBtn_clicked()
{
    ...
    if(!fileName.isEmpty())
    {
        ...
        ui->serverStatusLabel->setText(QString(u8"要传送的文件为：%1 ").arg(theFileName));
...
{
    tcpServer->close();
    ui->serverStatusLabel->setText(QString(u8"对方拒绝接收！！！"));
}
...
void TcpServer::on_serverSendBtn_clicked()
{
    ...
    ui->serverStatusLabel->setText(QString(u8"等待对方接收... ..."));
...
void TcpServer::initServer()
{
    ...
    ui->serverStatusLabel->setText(QString(u8"请选择要传送的文件"));
