...
{
    savePixmapAction = new QAction(QString::fromLocal8Bit("保存选区"),this);
    cancelAction = new QAction(QString::fromLocal8Bit("重选"),this);
...
{
    QString fileName;
    fileName = QFileDialog::getSaveFileName(this,QString::fromLocal8Bit("保存图片"),QDir::currentPath(),tr("Images (*.jpg *.png *.bmp)"));
    if(fileName.isNull())
    return;
...
void FullScreenWidget::drawTipsText()
{
    ...
    QString strTipsText = QString(QString::fromLocal8Bit("温馨提示\n鼠标拖动进行截屏;截屏区域内右键保存;\n截屏区域外右键取消;ESC退出;"));
