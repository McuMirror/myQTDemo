...
void frmIPC::on_btnAdd_clicked()
{
    ...
    if (IPCName == "") {
        myHelper::ShowMessageBoxError(QString::fromLocal8Bit("名称不能为空,请重新填写!"));
        ui->txtIPCName->setFocus();
        return;
    }

    if (NVRName == "") {
        myHelper::ShowMessageBoxError(QString::fromLocal8Bit("NVR名称不能为空,请先添加好NVR!"));
        return;
    }

    if (IPCRtspAddrMain == "") {
        myHelper::ShowMessageBoxError(QString::fromLocal8Bit("主码流地址不能为空,请重新填写!"));
        ui->txtIPCRtspAddrMain->setFocus();
        return;
    }

    if (IPCRtspAddrSub == "") {
        myHelper::ShowMessageBoxError(QString::fromLocal8Bit("子码流地址不能为空,请重新填写!"));
    ...
    if (IsExistIPCID(NVRID, IPCID)) {
        myHelper::ShowMessageBoxError(QString::fromLocal8Bit("编号已经存在,请重新选择!"));
        return;
    }
    ...
    ui->txtIPCName->setText(QString::fromLocal8Bit("摄像机%1").arg(ui->cboxIPCID->currentText()));
}

void frmIPC::on_btnDelete_clicked()
{
    if (ui->tableMain->currentIndex().row() < 0) {
        myHelper::ShowMessageBoxError(QString::fromLocal8Bit("请选择要删除的摄像机!"));
        return;
    }
    ...
    if (myHelper::ShowMessageBoxQuesion(QString::fromLocal8Bit("确定要删除摄像机吗?")) == 1) {
        QSqlQuery query;
...
{
    if (ui->tableMain->currentIndex().row() < 0) {
        myHelper::ShowMessageBoxError(QString::fromLocal8Bit("请选择要修改的摄像机!"));
        return;
    }
...
void frmIPC::on_btnExcel_clicked()
{
    ...
    if (myHelper::ShowMessageBoxQuesion(QString::fromLocal8Bit("导出数据到Excel成功,现在就打开吗？")) == 1) {
...
{
    ui->txtIPCName->setText(QString::fromLocal8Bit("摄像机%1").arg(arg1));
}
