...
{
    if (myHelper::ShowMessageBoxQuesion(QString::fromLocal8Bit("确定要清空NVR数据吗?")) == 1) {
        QSqlQuery query;
        QString sql = "delete from [NVRInfo] where [NVRID]!='000255'";
        query.exec(sql);
        myHelper::ShowMessageBoxInfo(QString::fromLocal8Bit("清空NVR数据成功!"));
    }
}

void frmConfig::on_btnClearIPC_clicked()
{
    if (myHelper::ShowMessageBoxQuesion(QString::fromLocal8Bit("确定要清空IPC数据吗?")) == 1) {
        QSqlQuery query;
        QString sql = "delete from [IPCInfo]";
        query.exec(sql);
        myHelper::ShowMessageBoxInfo(QString::fromLocal8Bit("清空IPC数据成功!"));
    }
}

void frmConfig::on_btnClearPoll_clicked()
{
    if (myHelper::ShowMessageBoxQuesion(QString::fromLocal8Bit("确定要清空轮询数据吗?")) == 1) {
        QSqlQuery query;
        QString sql = "delete from [PollInfo]";
        query.exec(sql);
        myHelper::ShowMessageBoxInfo(QString::fromLocal8Bit("清空轮询数据成功!"));
    }
}

void frmConfig::on_btnClearAll_clicked()
{
    if (myHelper::ShowMessageBoxQuesion(QString::fromLocal8Bit("确定要恢复出厂值吗?")) == 1) {
        QSqlQuery query;
        QString sql = "delete from [NVRInfo] where [NVRID]!='000255'";
        query.exec(sql);
        myHelper::Sleep(100);
        ...
        query.exec(sql);
        myHelper::ShowMessageBoxInfo(QString::fromLocal8Bit("恢复出厂值成功!"));
    }
}
