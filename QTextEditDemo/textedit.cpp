    ...
    ui(new Ui::TextEdit)
{
    ...
    left = new QAction(QIcon(":/images/textleft.png"),QString::fromLocal8Bit("左对齐"),actGrp);
    left->setCheckable(true);
    center = new QAction(QIcon(":/images/textcenter.png"),QString::fromLocal8Bit("居中对齐"),actGrp);
    center->setCheckable(true);
    right = new QAction(QIcon(":/images/textright.png"),QString::fromLocal8Bit("右对齐"),actGrp);
    right->setCheckable(true);
    justify = new QAction(QIcon(":/images/textjustify.png"),QString::fromLocal8Bit("两端对齐"),actGrp);
    ...
    //文本排序
    QLabel *listAlign = new QLabel(QString::fromLocal8Bit("排序："));
    listBox = new QComboBox(toolBar);
    //插入可选的排序方式
    listBox->addItem(QString::fromLocal8Bit("Standard"));
    listBox->addItem(QString::fromLocal8Bit("Bullet List (Disc)"));
    listBox->addItem(QString::fromLocal8Bit("Bullet List (Circle)"));
    listBox->addItem(QString::fromLocal8Bit("Bullet List (Square)"));
    listBox->addItem(QString::fromLocal8Bit("1 2 3 ... (Decimal)"));
    listBox->addItem(QString::fromLocal8Bit("a b c ... (Alpha lower)"));
    listBox->addItem(QString::fromLocal8Bit("A B C ... (Alpha upper)"));
    listBox->addItem(QString::fromLocal8Bit("ⅰ ⅱ ⅲ ...(Roman lower)"));
    listBox->addItem(QString::fromLocal8Bit("Ⅰ Ⅱ Ⅲ ... (Roman upper)"));
    ...
    tabbedView = new QAction(QString::fromLocal8Bit("标签模式(&T)"),actViewMode);
    tabbedView->setCheckable(true);
    tabbedView->setChecked(true);//默认选择为标签模式
    windowView = new QAction(QString::fromLocal8Bit("窗口模式(&W)"),actViewMode);
    ...
    if(action)
    {
        ...
        if(!hasOpened)//如果未打开则打开该文件
        {
            ...
            if(child->loadFile(action->data().toString()))
            {
                ...
                secondStatuslabel->setText(QString::fromLocal8Bit("打开成功"));
                initConnect();//初始化关联
            }
        }
    }
...
void TextEdit::updateRecentFiles()//更新最近打开文件列表
{
    ...
    {
        QString text = QString::fromLocal8Bit("&%1 %2").arg(i+1).arg(files[i]);
...
void TextEdit::updateMenus()//更新菜单栏和标题的显示状态等
{
    ...
    firstStatuslabel->setText(hasMdiChild? firstStatuslabel->text():QString::fromLocal8Bit("欢迎使用多文档文本编辑器"));
    secondStatuslabel->setText(hasMdiChild? secondStatuslabel->text():QString::fromLocal8Bit("yafeilinux制作"));
...
void TextEdit::updatemenuSelectw()//更新“选择窗口”菜单
{
    ...
    for (int i = 0; i < windows.size(); ++i)
    {
        ...
        if (i < 9)
        {
            text = QString::fromLocal8Bit("&%1 %2").arg(i + 1)
...
void TextEdit::initStatusbar()//初始化状态栏
{
    ...
    firstStatuslabel->setText(QString::fromLocal8Bit("欢迎使用多文档文本编辑器")); //初始化内容
    secondStatuslabel->setText(QString::fromLocal8Bit("yafeilinux制作"));
    thirdStatuslabel->setText(QString::fromLocal8Bit("0000-00-00 00:00::00 星期 "));
}
...
void TextEdit::doCursorchanged()//获取光标位置，并显示在状态栏
{
    ...
    firstStatuslabel->setText(QString::fromLocal8Bit("%1行 %2列").arg(rowNum+1).arg(colNum+1));//在状态栏显示光标位置
...
{
    secondStatuslabel->setText(QString::fromLocal8Bit("正在修改"));
}
...
void TextEdit::on_action_New_triggered()//新建
{
    ...
    child->newFile();
    child->setVisible(true);
    secondStatuslabel->setText(QString::fromLocal8Bit("新建成功"));
    initConnect();//初始化关联
}
...
void TextEdit::on_action_Save_triggered()//保存
{
    ...
    {
        secondStatuslabel->setText(QString::fromLocal8Bit("保存成功"));
    }
}
...
void TextEdit::on_action_Asave_triggered()//另存为
{
    ...
    {
        secondStatuslabel->setText(QString::fromLocal8Bit("另存为成功"));
    }
}
...
void TextEdit::on_action_PDF_triggered()//输出PDF文档
{
    if(activeMdiChild())
    {
        ...
        if (!fileName.isEmpty())
        {
            ...
            secondStatuslabel->setText(QString::fromLocal8Bit("输出PDF文档成功"));
        }
    }
}
...
{
    QMessageBox box;
    box.setWindowTitle(QString::fromLocal8Bit("关于"));
    box.setIcon(QMessageBox::Information);
    box.setText(QString::fromLocal8Bit("本软件由yafeilinux制作，是开源、共享、自由软件\n可以在网上传播、交流和下载，但请不要用作商业用途!\n\n多文档文本编辑器V1.0 Beta\n问题交流与BUG报告请联系：www.yafeilinux.com"));
    box.addButton(QString::fromLocal8Bit("确定"),QMessageBox::AcceptRole);//加入自定义的按钮
...
{
    QMessageBox::aboutQt(this,QString::fromLocal8Bit("关于Qt"));
}
...
void TextEdit::doFind(QString findText)//查找的实现函数
{
    if( (!senceF) && (!upFindF) )//不区分大小写，向后查找
    {
        ...
        {
            QMessageBox::warning(this,QString::fromLocal8Bit("查找"),QString::fromLocal8Bit("找不到 %1").arg(findText));
        }
    }
...
void TextEdit::on_action_Find_triggered()//显示查找对话框
{
    ...
    secondStatuslabel->setText(QString::fromLocal8Bit("正在查找"));
...
void TextEdit::on_action_Replace_triggered()//显示替换对话框
{
    ...
    secondStatuslabel->setText(QString::fromLocal8Bit("正在替换"));
}
