...
void MyMdi::NewFile()
{
    ...
    is_saved_ = false;
    current_file_path_ = QString::fromLocal8Bit("未命名文档%1.txt").arg(sequence_number++);
...
bool MyMdi::LoadFile(const QString &file_name)
{
    ...
    {
        //打开失败时，输出错误信息
        QMessageBox::warning(this, "多文档编辑器", QString::fromLocal8Bit("无法读取文件 %1：\n%2").arg(file_name).arg(file.errorString()));
        return false;
    }
...
bool MyMdi::has_saved()
{
    if(document()->isModified())
    {
        ...
        box.setText(QString::fromLocal8Bit("是否保存对%1的更改？").arg(get_current_file_name()));
        box.setIcon(QMessageBox::Warning);//警告图标
        //下面是消息box上添加3个按钮，分别为yes，no，cancel
        QPushButton *yes_button = box.addButton(QString::fromLocal8Bit("是"), QMessageBox::YesRole);
        QPushButton *no_button = box.addButton(QString::fromLocal8Bit("否"), QMessageBox::NoRole);
        QPushButton *cancel_button = box.addButton(QString::fromLocal8Bit("取消"), QMessageBox::RejectRole);
...
bool MyMdi::SaveAs()
{
    ...
    QString file_name = QFileDialog::getSaveFileName(this, QString::fromLocal8Bit("另存为"), current_file_path_);
...
bool MyMdi::SaveFile(const QString &file_name)
{
    ...
    {
        QMessageBox::warning(this, "多文档编辑器", QString::fromLocal8Bit("无法写入文件 %1：\n%2").arg(file_name).arg(file.errorString()));
        return false;
    }
...
void MyMdi::contextMenuEvent(QContextMenuEvent *event)
{
    ...
    QAction *undo = menu->addAction(QString::fromLocal8Bit("撤销(&U)"), this, SLOT(undo()), QKeySequence::Undo);//直接调用槽函数undo()
    undo->setEnabled(document()->isUndoAvailable());//因为该类是一个widget，所以可以直接使用document()函数

    QAction *redo = menu->addAction(QString::fromLocal8Bit("恢复(&A)"), this, SLOT(redo()), QKeySequence::Redo);
    ...
    QAction *select_all = menu->addAction(QString::fromLocal8Bit("全选"), this, SLOT(selectAll()), QKeySequence::SelectAll);
