    ...
    if(!file_name.isEmpty())
        {
            ...
            if(open_window->LoadFile(file_name))
                {
                    ui->statusBar->showMessage(QString::fromLocal8Bit("打开文件成功"), 2000);//状态栏显示打开文件成功，持续2秒
                    open_window->show();
                }
            else
                {
...
void MainWindow::on_actionSave_triggered()
{
    if(GetActiveWindow() && GetActiveWindow()->Save())
        ui->statusBar->showMessage(QString::fromLocal8Bit("保存文件成功"), 2000);//状态栏显示保存成功字样2秒
...
{
    QMessageBox::about(this, QString::fromLocal8Bit("关于本软件"), QString::fromLocal8Bit("参考www.yafeilinux.com网站做的一个实验"));
}
...
void MainWindow::ShowTextRowCol()
{
    if(GetActiveWindow())
        {
            ui->statusBar->showMessage(QString::fromLocal8Bit("%1行 %2列").arg(GetActiveWindow()->textCursor().blockNumber()+1).
    ...
    for(int i = 0; i < windows.size(); i++)
        {
            ...
            if(i < 1)//这个时候变化数字就是其快捷键
                text = QString::fromLocal8Bit("&% 1%2").arg(i+1).arg(child->get_current_file_name());//内容前面加了“&”表示可以使用快捷键，为第一个字母或数字
            else
                text = QString::fromLocal8Bit("%1 %2").arg(i+1).arg(child->get_current_file_name());
...
{
    setWindowTitle(QString::fromLocal8Bit("Simple Mutidocument Editor"));
    ui->mainToolBar->setWindowTitle(QString::fromLocal8Bit("工具栏"));//设置工具栏的标题名称，右击时才可以看到
    ...
    label->setText(QString::fromLocal8Bit("<a href = \"www.cnblogs.com/tornadomeet\">www.cnblogs.com/tornadomeet</a>"));//设置文本内容
